<div>
    ...
    <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
            ...
            <h1 class="text-2xl font-bold text-zinc-900 dark:text-white">New PCN Case</h1>
        </div>
        <div class="flex items-center gap-2">
            <a href="{{ route('flux-admin.pcn.index') }}" wire:navigate>
                <flux:button variant="ghost" size="sm" class="!rounded-none">Cancel</flux:button>
            </a>
            <flux:button wire:click="save" variant="primary" size="sm" class="!rounded-none">Save case</flux:button>
        </div>
    </div>
    ...
    <form wire:submit.prevent="save" class="space-y-6" novalidate>
        ...
        <div class="border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900 p-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                <x-flux-admin::field-group label="PCN number" required :error="$errors->first('form.pcn_number')">
                    <flux:input wire:model="form.pcn_number" placeholder="e.g. PCN12345678" />
                </x-flux-admin::field-group>
                    ...
                    @foreach($caseUpdates as $idx => $upd)
                        <div class="p-5 space-y-3" wire:key="upd-{{ $idx }}">
                            <div class="flex items-center justify-between">
                                <span class="text-xs font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-wide">Update #{{ $idx + 1 }}</span>
        ...
        <div class="flex justify-end gap-3 pt-2">
            ...
            <flux:button type="submit" variant="primary" class="!rounded-none">Save case</flux:button>
        </div>
