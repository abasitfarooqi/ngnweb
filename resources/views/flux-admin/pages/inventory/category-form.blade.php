    ...
    <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <div class="flex items-center gap-2 text-sm text-zinc-500 dark:text-zinc-400 mb-1">
                <a href="{{ route('flux-admin.inventory-categories.index') }}" class="hover:text-zinc-700 dark:hover:text-zinc-200 transition">Categories</a>
                <span>/</span>
                <span>{{ $category ? 'Edit' : 'New category' }}</span>
            </div>
            <h1 class="text-2xl font-bold text-zinc-900 dark:text-white">{{ $category ? 'Edit category: '.$category->name : 'New category' }}</h1>
        </div>
        <div class="flex items-center gap-2">
            <a href="{{ route('flux-admin.inventory-categories.index') }}">
    ...
    <form wire:submit.prevent="save" class="space-y-5" novalidate>
        <div class="border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900 p-5">
            ...
            <div class="flux-admin-form-grid grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <x-flux-admin::field-group label="Parent category" :error="$errors->first('form.super_category_id')">
                    <flux:select wire:model="form.super_category_id" placeholder="None (top level)">
