<div>
    <x-flux-admin::data-table title="Oxford products" description="Supplier catalogue mirrored from Oxford Products.">
        <x-slot:actions>
            ...
            <a href="{{ route('flux-admin.oxford-products.create') }}" wire:navigate>
                    ...
                    <flux:table.row wire:key="op-{{ $r->id }}">
                        ...
                        <flux:table.cell>
                            <div class="flex items-center gap-1">
                                <a href="{{ route('flux-admin.oxford-products.edit', $r) }}" wire:navigate>
                                    <flux:button size="xs" variant="ghost" icon="pencil-square" class="!rounded-none">Edit</flux:button>
                                </a>
                                <flux:button size="xs" variant="danger" wire:click="delete({{ $r->id }})" wire:confirm="Delete this product?" icon="trash" class="!rounded-none">Delete</flux:button>
                            </div>
                        </flux:table.cell>
                    </flux:table.row>
                @empty
