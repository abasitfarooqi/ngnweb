    ...
    <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <div class="flex items-center gap-2 text-sm text-zinc-500 dark:text-zinc-400 mb-1">
                <a href="{{ route('flux-admin.inventory-models.index') }}" class="hover:text-zinc-700 dark:hover:text-zinc-200 transition">Product Models</a>
                <span>/</span>
                <span>{{ $inventoryModel ? 'Edit' : 'New model' }}</span>
            </div>
            <h1 class="text-2xl font-bold text-zinc-900 dark:text-white">{{ $inventoryModel ? 'Edit model: '.$inventoryModel->name : 'New product model' }}</h1>
        </div>
        <div class="flex items-center gap-2">
            <a href="{{ route('flux-admin.inventory-models.index') }}">
