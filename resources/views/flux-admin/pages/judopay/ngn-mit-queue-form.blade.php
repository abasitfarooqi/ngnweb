    ...
    <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <div class="flex items-center gap-2 text-sm text-zinc-500 dark:text-zinc-400 mb-1">
                <a href="{{ route('flux-admin.ngn-mit-queue.index') }}" class="hover:text-zinc-700 dark:hover:text-zinc-200 transition">NGN MIT queue</a>
                <span>/</span>
                <span>{{ $recordId ? 'Edit' : 'New' }}</span>
            </div>
            <h1 class="text-2xl font-bold text-zinc-900 dark:text-white">{{ $recordId ? 'Edit queue entry' : 'New queue entry' }}</h1>
        </div>
        <div class="flex items-center gap-2">
            <a href="{{ route('flux-admin.ngn-mit-queue.index') }}">
    ...
    <form wire:submit.prevent="save" class="space-y-6" novalidate>
        <div class="border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900 p-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <x-flux-admin::field-group label="Invoice number" required :error="$errors->first('form.invoice_number')">
                ...
                <x-flux-admin::field-group label="Attempt #" :error="$errors->first('form.mit_attempt')">
                    <flux:input type="number" min="1" wire:model="form.mit_attempt" />
                </x-flux-admin::field-group>
            </div>
