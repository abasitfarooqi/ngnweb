<div>
    <x-flux-admin::data-table title="Vehicle database" description="Full compliance list with fleet association status — same as legacy vehicle database (all rows).">
        <x-slot:actions>
            <x-flux-admin::export-button />
            <a href="{{ route('flux-admin.motorbike-compliance.preview') }}" wire:navigate>
                <flux:button size="sm" variant="outline" icon="document-magnifying-glass" class="!rounded-none">MOT / TAX compliance</flux:button>
            </a>
        </x-slot:actions>

        <x-slot:toolbar>
            <x-flux-admin::filter-bar search-placeholder="Search REG. no, make or model…">
                ...
                <div class="min-w-0 w-full sm:min-w-[10rem] sm:flex-1 lg:w-44 lg:flex-none">
                    <flux:select wire:model.live="filters.association_status" placeholder="Status">
            ...
            <flux:table.rows>
                @forelse($rows as $r)
                    <flux:table.row wire:key="comp-{{ $r->id }}">
                        <flux:table.cell class="text-zinc-500 dark:text-zinc-400">{{ $r->id }}</flux:table.cell>
                        <flux:table.cell class="font-mono text-xs font-medium text-zinc-900 dark:text-white">{{ $r->motorbike?->reg_no ?: '—' }}</flux:table.cell>
                        <flux:table.cell class="text-zinc-600 dark:text-zinc-400">{{ $r->motorbike?->make ?: '—' }}</flux:table.cell>
                        <flux:table.cell class="text-zinc-600 dark:text-zinc-400">{{ $r->motorbike?->model ?: '—' }}</flux:table.cell>
                        ...
                        <flux:table.cell class="text-zinc-600 dark:text-zinc-400">{{ $r->motorbike?->color ?: '—' }}</flux:table.cell>
                        <flux:table.cell>
                            <x-flux-admin::association-status :status="$r->association_status ?? ''" />
                        </flux:table.cell>
                        <flux:table.cell>
                ...
                @empty
                    <flux:table.row><flux:table.cell colspan="12" class="text-center py-8 text-zinc-500 dark:text-zinc-400">No records.</flux:table.cell></flux:table.row>
                @endforelse
            </flux:table.rows>
        </flux:table>
