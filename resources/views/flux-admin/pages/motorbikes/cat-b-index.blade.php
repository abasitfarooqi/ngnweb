<div>
    <x-flux-admin::data-table title="Category B motorbikes" description="Write-off / damaged motorbike log.">
        <x-slot:actions>
            ...
            <flux:button size="sm" variant="primary" icon="plus" :href="url(config('backpack.base.route_prefix').'/motorbike-cat-b/create')" class="!rounded-none">New Cat B entry</flux:button>
        </x-slot:actions>
            ...
            <flux:table.rows>
                @forelse($rows as $r)
                    <flux:table.row wire:key="catb-{{ $r->id }}">
                        ...
                        <flux:table.cell>
                            <flux:button size="xs" variant="ghost" :href="url(config('backpack.base.route_prefix').'/motorbike-cat-b/'.$r->id.'/edit')" icon="pencil-square" class="!rounded-none">Edit</flux:button>
                        </flux:table.cell>
                    </flux:table.row>
                @empty
