    ...
    <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <div class="flex items-center gap-2 text-sm text-zinc-500 dark:text-zinc-400 mb-1">
                <a href="{{ route('flux-admin.upload-document-links.index') }}" class="hover:text-zinc-700 dark:hover:text-zinc-200 transition">Upload document links</a>
                <span>/</span>
                <span>{{ $recordId ? 'Edit' : 'New' }}</span>
            </div>
            <h1 class="text-2xl font-bold text-zinc-900 dark:text-white">{{ $recordId ? 'Edit upload link' : 'New upload link' }}</h1>
        </div>
        <div class="flex items-center gap-2">
            <a href="{{ route('flux-admin.upload-document-links.index') }}">
    ...
    <form wire:submit.prevent="save" class="space-y-6" novalidate>
        <div class="border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900 p-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <x-flux-admin::field-group label="Booking ID" :error="$errors->first('form.booking_id')">
            ...
            <div class="mt-4">
                <x-flux-admin::field-group label="Expires at" required :error="$errors->first('form.expires_at')">
