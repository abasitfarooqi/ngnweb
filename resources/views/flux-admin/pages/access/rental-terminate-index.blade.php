<div>
    ...
    <flux:modal wire:model.self="showForm" class="md:w-[560px]">
        <form wire:submit.prevent="saveForm" class="space-y-4">
            <flux:heading size="lg">{{ $recordId ? 'Edit link' : 'New terminate link' }}</flux:heading>
            ...
            <x-flux-admin::field-group label="Expires at" :error="$errors->first('formData.expire_at')" required>
                <flux:input type="datetime-local" wire:model="formData.expire_at" />
            </x-flux-admin::field-group>
