<div>
    <x-flux-admin::data-table title="Support messages" description="Flat log of every message across support conversations.">
        <x-slot:actions>
            <a href="{{ route('flux-admin.support-messages.create') }}"><flux:button size="sm" variant="primary" icon="plus" class="!rounded-none">New message</flux:button></a>
        </x-slot:actions>
        <x-slot:toolbar>
                    ...
                    <flux:table.row wire:key="sm-{{ $r->id }}">
                        ...
                        <flux:table.cell>
                            <div class="flex items-center gap-1">
                                <a href="{{ route('flux-admin.support-messages.edit', $r->id) }}"><flux:button size="xs" variant="ghost" icon="pencil-square" class="!rounded-none">Edit</flux:button></a>
                                <flux:button size="xs" variant="ghost" wire:click="delete({{ $r->id }})" wire:confirm="Delete this message?" icon="trash" class="!rounded-none text-red-600 dark:text-red-400">Delete</flux:button>
                            </div>
                        </flux:table.cell>
                    </flux:table.row>
                @empty
                    <flux:table.row><flux:table.cell colspan="7" class="text-center py-8 text-zinc-500 dark:text-zinc-400">No messages.</flux:table.cell></flux:table.row>
                @endforelse
            </flux:table.rows>
        </flux:table>
