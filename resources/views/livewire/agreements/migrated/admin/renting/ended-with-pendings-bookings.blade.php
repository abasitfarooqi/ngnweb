                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <p class="text-muted font-14 mb-3">
                            Ended with pendings (Proceed anyway) — collect remaining amounts via PAYMENT and CHARGES tabs.
                            <a href="{{ route('admin.renting.bookings.inactive') }}">Inactive bookings</a>
                            |
                            <a href="{{ route('admin.renting.bookings') }}">Active bookings</a>
                        </p>
                            ...
                            <table class="table mb-0 table-font">
                                <thead>
                                    <tr class="tr-booking-list">
                                        ...
                                        <th>PROCEEDED AT</th>
                                        <th>RENTAL LEFT</th>
                                        <th>ADDITIONAL LEFT</th>
                                        <th>PCN LEFT</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {{-- Ended with pendings: who proceeded and what is left to pay --}}
                                    @foreach ($bookingDetails as $booking)
                                        @php
                                            $stateClass = '';
                                            $stateClass_square = '';
                                            $today = new DateTime();
                                            $dueDate = !empty($booking->NEXT_DUE_DATE) ? new DateTime($booking->NEXT_DUE_DATE) : $today;
                                        ...
                                        <tr class="{{ $rowClass }} booking-row">
                                            ...
                                            <td class="middleleft">{{ $booking->PROCEEDED_BY ?? '—' }}</td>
                                            <td class="middleleft">{{ $booking->PROCEEDED_AT ?? '—' }}</td>
                                            <td class="middleleft">£{{ number_format($booking->RENTAL_LEFT ?? 0, 2) }}</td>
                                            <td class="middleleft">£{{ number_format($booking->ADDITIONAL_LEFT ?? 0, 2) }}</td>
                                            <td class="middleleft">£{{ number_format($booking->PCN_LEFT ?? 0, 2) }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
                                    ...
                                    <div id="motorbike-closing">
                                        <div class="container">
                                            ...
                                            <div class="row">
                                                ...
                                                <div class="col-md-4">
                                                    <label for="pending-total" id="pending-total-label"
            ...
            $('.booking-row').click(function() {
                ...
                if (selected_state === 'Completed') {
                    ...
                    template =
                        ...
                        '   <label class="form-check-label" for="sutton">Sutton</label>' +
                        '</div>' +
                        '<button class="btn btn-success mt-3" id="btn-management-reissue">INSPECT & ISSUE</button>';
                ...
                // 7.3, 7.4, 7.5 - CLOSING TAB AMOUNTS (same endpoint as modal - getClosingPendings)
                $.ajax({
                    url: '/admin/renting/booking/' + bookingId + '/closing-pendings',
                    type: 'GET',
                    success: function(response) {
                        console.log('Closing Pendings:', response);
                        var addTotal = (parseFloat(response.additional_charges_total) || parseFloat(response.additional_charges_due) || 0).toFixed(2);
                        var addPaid = (parseFloat(response.additional_charges_paid) || 0).toFixed(2);
                        var pcnTotal = (parseFloat(response.pcn_total) || parseFloat(response.pcn_due) || 0).toFixed(2);
                        var pcnPaid = (parseFloat(response.pcn_paid) || 0).toFixed(2);
                        var pendingRent = (parseFloat(response.pending_rent_due) || 0).toFixed(2);
                        $('#damages-total-label').text('TOTAL: £' + addTotal);
                        $('#damages-received-label').text('RECEIVED: £' + addPaid);
                        $('#damages-total').val(addTotal);
                        $('#damages-received').val(addPaid);
                        $('#pcn-total-label').text('TOTAL: £' + pcnTotal);
                        $('#pcn-received-label').text('RECEIVED: £' + pcnPaid);
                        $('#pcn-total').val(pcnTotal);
                        $('#pcn-received').val(pcnPaid);
                        $('#pending-total-label').text('PENDING TOTAL: £' + pendingRent);
                        $('#pending-total').val(pendingRent);
                    },
                    error: function(xhr, status, error) {
                        console.log(`Closing pendings Error: ${error}`);
                        $('#damages-total-label').text('TOTAL: £0.00');
                        $('#damages-received-label').text('RECEIVED: £0.00');
                        $('#pcn-total-label').text('TOTAL: £0.00');
                        $('#pcn-received-label').text('RECEIVED: £0.00');
                        $('#pending-total-label').text('PENDING TOTAL: £0.00');
                    }
                });
                    ...
                    success: function(response) {
                        ...
                        if (response.success === false) {
                            ...
                            $('#payment-invoices').append(
                                ...
                                <td>
                                ${invoice.INV_STATE === 'Completed' ? '<button class="btn btn-success" disabled>Paid</button>' : '<label class="btn btn-danger" name="" id="" disabled>UnPaid</label>'}
                                </td>
                                </tr>
                                `).join('')}
                                        </tbody>
