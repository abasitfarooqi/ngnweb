...
<flux:sidebar sticky stashable class="bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800">
    ...
    <div class="px-4 py-3 border-b border-gray-200 dark:border-gray-800">
        ...
        @if($verificationBadge ?? false)
            <flux:badge color="{{ $verificationBadge === 'verified' ? 'green' : 'yellow' }}" class="mt-1 text-xs">
                {{ ucfirst($verificationBadge) }}
            </flux:badge>
        @endif
    </div>
    ...
    <flux:navlist class="flex-1 py-2">
        ...
        <flux:navlist.item icon="home" href="{{ route('account.dashboard') }}" :current="request()->routeIs('account.dashboard')">
            Dashboard
        </flux:navlist.item>

        <flux:navlist.item icon="user" href="{{ route('account.profile') }}" :current="request()->routeIs('account.profile')">
            Profile
        </flux:navlist.item>

        <flux:navlist.item icon="document-text" href="{{ route('account.documents') }}" :current="request()->routeIs('account.documents')">
            Documents
        </flux:navlist.item>

        <flux:navlist.group icon="calendar" heading="Repairs & MOT" :expandable="true"
        ...
        <flux:navlist.group icon="clock" heading="Rentals" :expandable="true" :expanded="request()->routeIs('account.rentals*')">
        ...
        <flux:navlist.group icon="banknotes" heading="Finance" :expandable="true" :expanded="request()->routeIs('account.finance*')">
        ...
        <flux:navlist.group icon="bolt" heading="Recovery" :expandable="true" :expanded="request()->routeIs('account.recovery*')">
        ...
        <flux:navlist.item icon="shopping-bag" href="{{ route('account.orders') }}" :current="request()->routeIs('account.orders')">Orders</flux:navlist.item>
        <flux:navlist.item icon="star" href="{{ route('account.club') }}" :current="request()->routeIs('account.club')">NGN Club</flux:navlist.item>
        <flux:navlist.item icon="shield-check" href="{{ route('account.security') }}" :current="request()->routeIs('account.security')">Security</flux:navlist.item>

    </flux:navlist>

    <div class="border-t border-gray-200 dark:border-gray-800 p-4">
        <a href="/" class="flex items-center gap-2 text-xs text-gray-500 hover:text-brand-red mb-3 transition">
            ← Back to NGN Site
        </a>
        <form method="POST" action="{{ route('logout') }}">
            @csrf
            <button type="submit" class="flex items-center gap-2 text-xs text-gray-500 hover:text-brand-red transition">
...
<flux:main class="flex-1 min-h-screen">
    ...
    <div class="lg:hidden flex items-center justify-between px-4 py-3 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800">
        ...
        <form method="POST" action="{{ route('logout') }}">@csrf
            <button class="text-xs text-gray-500 hover:text-brand-red">Sign out</button>
        </form>
    </div>
