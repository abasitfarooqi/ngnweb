<x-emails.base :subject="$subject ?? 'NGN Motors Update'">
    ...
    @if (!empty($bodyHtml ?? null))
        <div class="email-legacy-html email-text" style="margin:0 0 22px;font-size:14px;color:#111827!important;line-height:1.75;">
            {!! $bodyHtml !!}
        </div>
    @endif

    @foreach ($intro as $line)
        <p class="email-text" style="margin:0 0 12px;font-size:14px;color:#111827!important;line-height:1.7;">
            {{ $line }}
        </p>
    @endforeach
    ...
    @if (!empty($detailsMap))
        <table width="100%" border="0" cellpadding="0" cellspacing="0" style="border:1px solid #e5e7eb!important;margin:0 0 24px;border-collapse:collapse;">
            @foreach ($detailsMap as $label => $value)
                <tr>
                    <td style="padding:10px 12px;font-size:11px;font-weight:700;color:#111827!important;background-color:#f9fafb!important;border-bottom:1px solid #e5e7eb!important;width:34%;vertical-align:top;text-transform:uppercase;letter-spacing:0.06em;">
                        {{ $label }}
                    </td>
                    <td style="padding:10px 12px;font-size:13px;color:#111827!important;border-bottom:1px solid #e5e7eb!important;vertical-align:top;line-height:1.5;">
                        {{ $value }}
                    </td>
                </tr>
            @endforeach
    ...
    @if (!empty($actionUrl ?? null))
        <p class="email-muted" style="margin:0 0 4px;font-size:12px;color:#4b5563!important;line-height:1.6;">
            If the button does not work, copy and paste this link:
        </p>
        <p style="margin:0 0 20px;font-size:12px;word-break:break-all;">
            <a href="{{ $actionUrl }}" style="color:#c31924!important;text-decoration:underline;">{{ $actionUrl }}</a>
        </p>
    @endif

    @foreach ($outro as $line)
        <p class="email-text" style="margin:0 0 10px;font-size:13px;color:#111827!important;line-height:1.7;">
            {{ $line }}
        </p>
    @endforeach

    <table width="100%" border="0" cellpadding="0" cellspacing="0" style="margin:22px 0 16px;">
        <tr><td class="email-divider" style="border-top:1px solid #e5e7eb!important;font-size:0;line-height:0;">&nbsp;</td></tr>
    </table>

    <p class="email-muted" style="margin:0 0 10px;font-size:11px;color:#4b5563!important;line-height:1.6;text-transform:uppercase;letter-spacing:0.07em;">
        Branch contacts
    </p>
    <table width="100%" border="0" cellpadding="0" cellspacing="0" style="margin:0 0 6px;">
        <tr>
            <td style="padding:0 0 10px;font-size:12px;color:#374151!important;line-height:1.55;">
        ...
        <tr>
            <td style="padding:0;font-size:12px;color:#374151!important;line-height:1.55;">
