...
@section('after_scripts')
    ...
    $(document).ready(function() {
        // On brand change, fetch product types
        $('select[name="brand_id"]').on('change', function() {
            var brandId = $(this).val();
            $.ajax({
                url: '/get-product-types/' + brandId,
                method: 'GET',
                success: function(data) {
                    $('select[name="product_type_id"]').empty();
                    $.each(data, function(key, value) {
                        $('select[name="product_type_id"]').append('<option value="' + key + '">' + value + '</option>');
                    });
                    $('select[name="product_type_id"]').trigger('change');
                }
            });
        });

        // Additional dependent selects can follow the same structure
    });
    </script>
