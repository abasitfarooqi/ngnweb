...
class EcommerceViewResolver
{
    public function handle(Request $request, Closure $next)
    {
        ...
        if (! config('ecommerce.live') && ! config('ecommerce.portal')) {
            if ($isEcommercePath) {
                return response()->view('frontend.vue_store.shop_maintenance');
            }
            if ($isAccountPath) {
                return response()->view('frontend.vue_store.account_maintenance');
            }
        }
        ...
        return response()->view('frontend.vue_store.app');
    }
}
