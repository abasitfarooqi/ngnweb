...
class DiscoveredTransactionalCommunicationCatalog implements TransactionalCommunicationDefinitionProvider
{
    ...
    {
        return [
            $this->mail('portal.password.reset', 'Portal Password Reset', 'Secure password reset link sent to a customer portal user.', 'portal', \App\Mail\PortalPasswordResetMail::class, 'emails.portal.password-reset', 'Customer portal email', ['customer', 'reset_link']),
            $this->mail('portal.email.verification', 'Portal Email Verification', 'Email verification link sent to a customer portal user.', 'portal', \App\Mail\PortalEmailVerificationMail::class, 'emails.portal.email-verification', 'Customer portal email', ['customer', 'verification_link']),
            $this->mail('portal.credentials.issued', 'Portal Credentials Issued', 'Portal login credentials sent after access is created or reset.', 'portal', \App\Mail\PortalCredentialsMail::class, 'emails.portal.credentials', 'Customer portal email', ['customer', 'credentials']),
            ...
            $this->mail('contract.hire.issued', 'Sale Contract Issued', 'Sale contract email with generated contract attachments.', 'contracts', \App\Mail\HireContract::class, 'emails.templates.agreement-controller-universal', 'Customer email and existing configured copies', ['customer', 'contract', 'pdf'], mandatory: true),
            ...
            $this->mail('rental.invoice.update_reminder', 'Invoice chase note to the customer', 'Not sent. Invoice notes stay internal for staff. This mailer is kept in the catalogue only.', 'rentals', \App\Mail\RentingInvoiceUpdateReminderMail::class, 'emails.templates.agreement-controller-universal', 'Not sent', ['customer', 'booking', 'invoice', 'update']),
        ];
    }
