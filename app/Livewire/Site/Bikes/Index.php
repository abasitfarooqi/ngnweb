    ...
    public function render()
    {
        ...
        if (in_array($this->filterType, ['all', 'new'])) {
            try {
                ...
                if ($this->searchMake) $q->where('make', 'like', '%' . $this->searchMake . '%');
                $newBikes = $q->orderBy('created_at', 'desc')->take(12)->get();
            } catch (\Exception $e) { $newBikes = collect(); }
        }
        ...
        if (in_array($this->filterType, ['all', 'used'])) {
            try {
                ...
                if ($this->searchMake) $q->where('motorbikes.make', 'like', '%' . $this->searchMake . '%');
                if ($this->minPrice) $q->where('motorbikes_sale.price', '>=', $this->minPrice);
                if ($this->maxPrice) $q->where('motorbikes_sale.price', '<=', $this->maxPrice);
                $usedBikes = $q->orderBy('motorbikes.created_at', 'desc')->take(12)->get();
            } catch (\Exception $e) { $usedBikes = collect(); }
        }
