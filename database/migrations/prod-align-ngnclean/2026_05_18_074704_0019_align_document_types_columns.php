...
return new class extends Migration
{
    public function up(): void
    {
        ...
        if (! Schema::hasColumn('document_types', 'slug')) {
            DB::statement('ALTER TABLE `document_types` ADD COLUMN `slug` varchar(255) NOT NULL AFTER `name`');
        }
        ...
        if (! Schema::hasColumn('document_types', 'required_for')) {
            DB::statement('ALTER TABLE `document_types` ADD COLUMN `required_for` json NULL DEFAULT NULL AFTER `is_mandatory`');
        }

        if (! Schema::hasColumn('document_types', 'validation_rules')) {
            DB::statement('ALTER TABLE `document_types` ADD COLUMN `validation_rules` json NULL DEFAULT NULL AFTER `required_for`');
        }
